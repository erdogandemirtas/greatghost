...
public class TextureAtlasSlicer : EditorWindow
{
    ...
    private void PerformSlice()
    {
        ...
        // Meta verilerini güncellemek için SerializedObject ve SerializedProperty kullanýn
        var serializedObject = new SerializedObject(importer);
        var spritesheetProperty = serializedObject.FindProperty("m_Spritesheet");
        ...
        // Eski spritesheet ile karþýlaþtýrýn ve gerekirse güncelleyin
        if (spritesheetProperty != null && !spritesheetProperty.Equals(wantedSpriteSheet))
        {
            needsUpdate = true;
            spritesheetProperty.ClearArray();
            for (int i = 0; i < wantedSpriteSheet.Length; i++)
            {
                spritesheetProperty.InsertArrayElementAtIndex(i);
                var element = spritesheetProperty.GetArrayElementAtIndex(i);
                element.FindPropertyRelative("name").stringValue = wantedSpriteSheet[i].name;
                element.FindPropertyRelative("rect").rectValue = wantedSpriteSheet[i].rect;
                element.FindPropertyRelative("alignment").intValue = wantedSpriteSheet[i].alignment;
                element.FindPropertyRelative("pivot").vector2Value = wantedSpriteSheet[i].pivot;
            }
        }
